...
from rclpy.rosbag2_py import SequentialReader
from rclpy.rosbag2_py import StorageOptions, ConverterOptions
import time
...
class CenterpointNode(Node):
    def __init__(self):
        ...
        # self.set_ros2bag(self, bag_path)
    ...
    def play_bag(self):
        while self.reader.has_next():
            ...
            # /lidar/top/pointcloud 토픽일 경우 퍼블리시
            if topic == '/lidar/top/pointcloud':
                msg = PointCloud2()
                msg.deserialize(data)
                self.publisher_.publish(msg)
                self.get_logger().info(f'Published message from {topic} at timestamp {t}')

            # 약간의 지연을 추가해 데이터 속도 조절 (실제 시간 기반)
            time.sleep(0.1)
...
#
# def read_pointcloud_from_bag(bag_path, topic_name="/lidar/top/pointcloud"):
...
#
#     # Get all topic types in the bag
...
#                 cloud_points = point_cloud2.read_points(msg, field_names=("x", "y", "z"), skip_nans=True)
#                 cloud_array = np.array(list(cloud_points))
#
#                 return cloud_array
#
...
def main(args=None):
    ...
    centerpoint_node.set_ros2bag(
        bag_path='/Dataset/HDD8TB/Datasets/waymo_rosbag2/segment-10359308928573410754_720_000_740_000_with_camera_labels')

    # Bag 파일 재생
    centerpoint_node.play_bag()

    rclpy.spin(centerpoint_node)
    centerpoint_node.destroy_node()
    rclpy.shutdown()
